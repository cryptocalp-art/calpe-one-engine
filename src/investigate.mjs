...
async function writeOutputs(investigations = [], claims = [], evidences = [], links = []) {
  ...
  await Promise.all([
    ...
    fs.writeFile("data/investigation-last-run.json", JSON.stringify({
      status:"success", generated_at:now, model, candidates_processed:candidates.length,
      verified:investigations.filter(x=>x.status==="VERIFIED").length,
      needs_research:investigations.filter(x=>x.status==="NEEDS_RESEARCH").length,
      rejected:investigations.filter(x=>x.status==="REJECTED").length
    }, null, 2) + "\n")
  ]);
}
...
if (!candidates.length) {
  await writeOutputs();
  console.log(JSON.stringify({ processed:0, verified:0, needs_research:0, rejected:0, api_skipped:true }, null, 2));
  process.exit(0);
}
...
await writeOutputs(investigations, claims, evidences, links);
